...
papichgif =[
    ...
    "https://media.tenor.com/wXSdSO_ilucAAAAC/papich-%D0%BF%D0%B0%D0%BF%D0%B8%D1%87.gif",
...
variantinastoeniebad =["плохо","что то не очень","sad","тильт","не очень","плоховато","печально","кринж","тільт","тілььт","тільььт",]
variantinastoeniagood=["хорошо", "отлично","прекрасно","супер","гуд","ультрамеганорм","найс","нойс"]
variatiprivetov = [
    "ку бот","хай бот","привет бот","здарова бот","доброе утро бот","здраствуй бот","привіт бот"
]
...
voda = ["https://vip.shuvar.com/media/catalog/product/cache/628b1a33a4779cd89563027f2a2c1a58/V/o/Voda-Morshynska-slabohazovana-15l.png"]
taro = ["https://avatars.dzeninfra.ru/get-zen_doc/5236207/pub_63383af93c55cc1557ed0f09_63383afd71a6914e41bd151b/scale_1200",
        "https://avatars.dzeninfra.ru/get-zen_doc/5097825/pub_63383af93c55cc1557ed0f09_63383afdd64c9a5b9c82c364/scale_1200",
        "https://avatars.dzeninfra.ru/get-zen_doc/5283638/pub_63383af93c55cc1557ed0f09_63383afdb741fb6da17df75e/scale_1200",
        "https://avatars.dzeninfra.ru/get-zen_doc/3420563/pub_63383af93c55cc1557ed0f09_63383affb84d20324af89170/scale_1200",
        "https://avatars.dzeninfra.ru/get-zen_doc/5226854/pub_63383af93c55cc1557ed0f09_63383afffba3c50ea73a9730/scale_1200",
        "https://sun9-27.userapi.com/impf/c858120/v858120082/67bcc/u6g-GU2iJwU.jpg?size=207x340&quality=96&sign=235b29d4f3d654125d38e9e9a9ffcf4e&type=album",
        "https://www.meme-arsenal.com/memes/2717e32d174c4bce19ef7e43531d26b9.jpg",
        "https://avatars.dzeninfra.ru/get-zen_doc/1587012/pub_63383af93c55cc1557ed0f09_63383b00e77ddd33767e6192/scale_1200",
        "https://astrohelper.ru/images/gadaniya/arkany/sila.jpg",
        "https://raw.githubusercontent.com/shlyapnikova/mytetra/master/base/1525262748koy182lhhu/image10054.png",
        "https://avatars.dzeninfra.ru/get-zen_doc/3855260/pub_5fe5de33e5cdbc6a9679dddd_5fe5de61e5cdbc6a967a2e14/scale_1200",
        "https://i.pinimg.com/564x/2a/ed/fc/2aedfce767129f6627b32ebad7ec3815.jpg",
        "https://tayniymir.com/wp-content/uploads/2017/02/1-11.jpg",
        "https://avatars.dzeninfra.ru/get-zen_doc/1587012/pub_63383af93c55cc1557ed0f09_63383b03df5d464b37273143/scale_1200",
        "https://avatars.dzeninfra.ru/get-zen_doc/5232141/pub_63383af93c55cc1557ed0f09_63383b04988e2f102f93f66b/scale_1200",
        "https://sun9-69.userapi.com/impg/M7rcM-DRwHPopgnRqkoQv0nhaUEZxzeYMT2eCg/TwHr79TqYlU.jpg?size=423x807&quality=96&sign=d845dcf632183b85dc7c3e1570976f3f&type=album",
        "https://avatars.dzeninfra.ru/get-zen_doc/3965742/pub_5ffd4b827cd87011f0b2f430_5ffd4b85aeef3c78296bbaff/scale_1200",

        ]
ukr = ["чому не державною?","руснявий бот","не андерстуд"," не андерстенд","вийди от сюда розбійник","погана русня"]
...
async def on_ready():
    ...
    async def kvest(ctx):
        ...
        if responce.component.label == "Не":
            await responce.respond(embed=discord.Embed(title="Ты злюкаааа(но мне всё равно)"),
                                   components=[
        ...
        if responce.component.label == "Продолжить":
            await responce.respond(
                embed=discord.Embed(
                    title="У меня в запасе минут 20, может захватить воды?").set_image(url=random.choice(voda)),
                components=[
        ...
        if responce.component.label == "Продолжить":
            await responce.respond(
                embed=discord.Embed(
                    title="Скорее напоминающие заброшенный завод чем офис. Всё по заветам серпа и молота.Впрочем внутри не так плачевно как снаружи, так что жаловатся толком не на что."),
                components=[
                    Button(style=ButtonStyle.blue, label="Продолжить"),
                ]),
        responce = await bot.wait_for("button_click")
        ...
        if responce.component.label == "Продолжить":
            ...
            if responce.component.label == "Молча кивнуть в знак согласия":
                await responce.respond(embed=discord.Embed(
                    title="«Ну что кєп раньше начнём, раньше закончим»."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Но для ученых понимание, даже приблизительное  как это может функционировать, даст очень большой толчок для создания такого проекта в нашей жизни. Может сначала и уменьшаться масштаб до 10 тыс. человек."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
                    ]),
            responce = await bot.wait_for("button_click")
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Севши в кресло и одевши обруч для подключения на виртуальную станцию «Омега», я дал знак Яру и закрыл глаза.По плану я буду кораблём снабжения с Земли, который привёз необходимые для жизни продукты и вещи"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
                    ]),
            responce = await bot.wait_for("button_click")
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="За день разгружусь,после еще один день проведу в исследовании станции.После сяду на тот же корабль и вернусь на Землю\в реальных мир.Либо если не получится вылететь вечером второго дня,ночью с второго дня на третий меня принудительно выведет Яр."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
                    ]),
            responce = await bot.wait_for("button_click")
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Сухо в горле.Сделавши глотов из бутылки которую удачно купил утром я закрыл глаза и сказал-«Начать подключение»"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
                    ]),
            if responce.component.label == "Продолжить":
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Выйдя из корабля я видел такой футористичный, но такой знакомый пейзаж города будущего. Правда из того самого мрачного где победили корпорации и т.д. , а не счастливый техно-скачек."),
                    components=[
                                   Button(style=ButtonStyle.blue, label="Продолжить"),
                        ]),
            responce = await bot.wait_for("button_click")
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Темная прямо-геометрическая застройка без капельки дизайнерских решений. Благодаря искусственной гравитации, сквозь симуляцию эко сферы, а точнее симуляции неба и туч виднелись перевёрнутые дома над головой."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
                    ]),
            responce = await bot.wait_for("button_click")
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title=" ГГ-«Кораблям ставили ставить улучшенные кинетические щиты. Если что и прилетает, максимум потрясёт немного. Крайне удачная модель.»"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Спустившись, я увидел огромный атриум и один столик у которого сотрудник СБО что то обговаривал с местным. Прервав их беседу узнал куда мне нужно. Сотрудник показал рукой на дверь справа."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Выйдя из лифта я обнаружил коридор, дверь и ресепшн около неё. Подойдя к двери мужчина на за стойкой не обратил на меня никакого внимания, так что я провёл карточкой и зашел внутрь."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Ангар выглядел как огромный прямоугольник без одного ребра. Сам зал делился на подобие 2 этажей с закрепленными с помощью захватов аэрокарами. Вся инфраструктура станции это лифты и аэрокары."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Опустившись на местной площадке для каров, я покинул автомобиль и подошел к терминалу помощи. Терминал стоял прям на выходе из места приземления. "),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Просмотреть новости станции, узнать о планируемых мероприятиях, для местных посмотреть ценны. В общем универсальный помощник во всём."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="В ходе разговора узнал что бармена звали  Мордин, а эго собеседника Явик. Разговор шел достаточно бодро и местная выпивка этому способствовала. Спустя определённое время я всё таки решил спросить."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="ГГ-«Слышали про аварию в секторе H-32, что там могло случится что аж закрыли гражданский сектор. Это ж надо перераспределить целый сектор заново. Сколько я летаю такого не помню.»"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Мордин-«И ты в это веришь? Еще бы рассказал что там пришельцы захватили корпус и строят армию. Тьфу. Я скорее верю в неудачную аварию каров и за чего там очень большие завалы.»"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Я попрощался с парнями и пошел к себе отсыпаться. По пути думал: «Почему Alife решил скрыть причину и последствия аварии?». Он конечно сам решает что ему нужно, но раньше он так не делал."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Вкусно позавтракав и расплатившись, я пошел на «границу» секторов. Может получится узнать что то заранее"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Тех.сотрудник –«Ну скорее для пользы всей станции чем меня. Впрочем, я её получил сегодня утром и сразу отправился сюда, ведь и за одной маленькой неприятности отключён целый жилой сектор.»"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Подойдя к терминалу и отрыв карту сектора H-32, которая к удивлению была не заблокирована и общедоступна. Еще одна недоработка AI. "),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Я отправился проверить кампус, если там пусто я ничего не теряю, а так получу больше информации зачем Alife скрывает аварию. Инженера на границе уже не было и я спокойной зашел в сектор H-32"),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(
                    embed=discord.Embed(
                        title="Зайдя в кабинет, я увидел кристалл неизвестного материале которой отражал внутри себя кучу цветов одновременно и отображая их на весь кабинет."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Не трогать кристалл":
                await responce.respond(embed=discord.Embed(
                    title="Крайне глупо торкаться то что устроило этот хаос. Вокруг меня было куча документов понять которые абсолютно не является возможным. Все было абсолютно хаотично разбросанно."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Из обрывка документа стало известно что кристалл гранился за огромной кучей преград. Две экранирующие сетки, металлический саркофаг и тройной слой стекла сверху."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
            ...
            if responce.component.label == "Продолжить":
                await responce.respond(embed=discord.Embed(
                    title="Прилетев в ангар я зашел в лифт и выбрал этаж 2 «Док D-2». Выйдя из лифта я застыл на месте."),
                    components=[
                        Button(style=ButtonStyle.blue, label="Продолжить"),
...
async def on_message(message):
    ...
    if msg in variatiprivetov:
     await message.channel.send(f'Хаааай {message.author.name}. Як справи?')
    if msg in ukr:
        await message.channel.send(f'Хаааай {message.author.name}. Як справи?')
    ...
    if msg in variantinastoeniebad:
        await message.channel.send(f'Тільта не будєєттт(я забороняю(тільки в крайніх випадках))')
        await bot.process_commands(message)
...
async def ping(ctx):
    ...
    if responce.component.label == "Не":
        await responce.respond(embed=discord.Embed(
            title="Челнок").set_image(url="https://static.wikia.nocookie.net/masseffect/images/9/95/Codex_UT-47A_Kodiak.png/revision/latest/scale-to-width-down/250?cb=20130515150204&path-prefix=ru"),

        components=[Button(style=ButtonStyle.green,label="Next")]
        ),

@bot.command()
async def Taro(ctx):
    await ctx.send(
        embed=discord.Embed(title="Карти дня"),
        components=[

            Button(style=ButtonStyle.red, label="ага"),
        ]
    )
    responce = await bot.wait_for("interaction")
    if responce.component.label == "ага":
        await responce.send(embed=discord.Embed(
            title="Перша карта").set_image(url=random.choice(taro)),

        components=[Button(style=ButtonStyle.green,label="наступна")]
        ),
        responce = await bot.wait_for("button_click")
        if responce.component.label == "наступна":
            await responce.send(embed=discord.Embed(
                title="Друга карта").set_image(url=random.choice(taro)),
